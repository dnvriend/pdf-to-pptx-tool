...
typecheck: ## Run type checking with mypy
	uv run mypy pdf_to_pptx_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r pdf_to_pptx_tool -c pyproject.toml
...
run: ## Run pdf-to-pptx-tool (usage: make run ARGS="...")
...
install-global: ## Install globally with uv tool
	uv tool uninstall pdf-to-pptx-tool || true
	uv build --force-pep517
	uv tool install .

uninstall-global: ## Uninstall global installation
	uv tool uninstall pdf-to-pptx-tool
